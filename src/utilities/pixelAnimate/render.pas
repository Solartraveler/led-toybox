...
uses
  Classes, SysUtils, DOM, StrUtils, dialogs, Math;
...
function addFadein(ch: TDOMNode; sx, sy, format: integer; lastFrameIdx: integer; var animation: TBuffer): integer;
function addFadeout(ch: TDOMNode; sx, sy, format: integer; lastFrameIdx: integer; var animation: TBuffer): integer;
function bytesPerPixel(format: integer): integer;

const progamVersion = '0.4.0';
...
begin
  ...
  if ((offset + pixelsize) <= animation.dataUsed) then begin
    if (pixelsize = 6) then begin
     ...
     //showmessage('Error getting pixel x:' + inttostr(x) + ' y:' + inttostr(y) +'. Offset ' + inttostr(offset + pixelsize) + ' out of limit ' + inttostr(animation.dataUsed));
     result.r := 0; //deterministic behaviour
     result.g := 0;
     result.b := 0;
  end;
end;
...
//Generated by ChatGPT based on addRoll
function addFadein(ch: TDOMNode; sx, sy, format: integer; lastFrameIdx: integer; var animation: TBuffer): integer;
var
  repeats, duration, x, y, i, currentFrameIdx: integer;
  t, factor: double;
  maxVal: integer;
  gamma: double;
  srcPixel, pixel: TPixel;
  original: array of TPixel;
begin
  result := lastFrameIdx;
  repeats := DomstrToInt(ch.Attributes.GetNamedItem('repeats').NodeValue);
  duration := DomstrToInt(ch.Attributes.GetNamedItem('duration').NodeValue);
  gamma := DomstrToFloat(ch.Attributes.GetNamedItem('gamma').NodeValue);
  maxVal := (1 shl format) - 1;

  // store original frame
  original := nil;
  setLength(original, sx * sy);
  for y := 0 to sy - 1 do begin
    for x := 0 to sx - 1 do begin
      original[y * sx + x] := getPixel(x, y, lastFrameIdx, format, sx, animation);
    end;
  end;
  // rewrite previous frame to black
  pixel.r := 0;
  pixel.g := 0;
  pixel.b := 0;
  pixel.format := format;
  for y := 0 to sy - 1 do begin
    for x := 0 to sx - 1 do begin
      setPixel(x, y, lastFrameIdx, sx, sy, pixel, animation);
    end;
  end;
  ...
  for i := 0 to repeats - 1 do begin
    ...
    for y := 0 to sy - 1 do
      for x := 0 to sx - 1 do begin
        srcPixel := original[y * sx + x];
        pixel.r := round(srcPixel.r * factor);
        pixel.g := round(srcPixel.g * factor);
        pixel.b := round(srcPixel.b * factor);
        pixel.format := format;

        if pixel.r > maxVal then pixel.r := maxVal;
        if pixel.g > maxVal then pixel.g := maxVal;
        if pixel.b > maxVal then pixel.b := maxVal;
...
begin
  ...
  for i := 0 to repeats - 1 do begin
    ...
    for y := 0 to sy - 1 do
      for x := 0 to sx - 1 do begin
        pixel := getPixel(x, y, lastFrameIdx, format, sx, animation);

        pixel.r := round(pixel.r * factor);
        pixel.g := round(pixel.g * factor);
        pixel.b := round(pixel.b * factor);
...
begin
  ...
  while (assigned(ch)) do begin
    ...
    end else if (ch.NodeName = 'fadein') then begin
      lastFrameIdx := addFadein(ch, sx, sy, format, lastFrameIdx, animation);
    end else if (ch.NodeName = 'fadeout') then begin
      lastFrameIdx := addFadeout(ch, sx, sy, format, lastFrameIdx, animation);
    end;
    ch := ch.NextSibling;
  end;
  //append termination
