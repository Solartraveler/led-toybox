...
void MatrixStop(void) {
	if (TIM1->CR1 & TIM_CR1_CEN) {
		//wait for the frame to be done to remove any flicker
		printf("Wait for CNT to be 0\r\n");
		while(TIM8->CNT == 0);
		printf("Wait for CNT to be non 0\r\n");
		while(TIM8->CNT != 0); //so now TIM5 should be very near to 0 too
	}
	TIM1->CR1 = 0;
	TIM8->CR1 = 0;
	DMA2_Stream5->CR &= ~DMA_SxCR_EN;
	printf("Wait for DMA 2-5 to be done\r\n");
	while (DMA2_Stream5->CR & DMA_SxCR_EN);

	DMA2_Stream1->CR &= ~DMA_SxCR_EN;
	printf("Wait for DMA 2-1 to be done\r\n");
...
void MatrixStart(void) {
	...
	*/
	...
	DMA2_Stream5->PAR = (uint32_t)&((R10_GPIO_Port->ODR));
	DMA2_Stream5->M0AR = (uint32_t)g_matrixBuffer;
	DMA2_Stream5->NDTR = transfersX;
	//TIM1_UP channel, memory->periph, 16Bit, circular
	DMA2_Stream5->CR = (6 << DMA_SxCR_CHSEL_Pos) | DMA_SxCR_DIR_0 | DMA_SxCR_MINC |
	...
	DMA2_Stream1->PAR = (uint32_t)&(Line1_GPIO_Port->BSRR);
	DMA2_Stream1->M0AR = (uint32_t)g_lineDriver;
	DMA2_Stream1->NDTR = transfersY;
	//TIM8_UP, memory->periph, 32Bit, circular
	DMA2_Stream1->CR = (7 << DMA_SxCR_CHSEL_Pos) | DMA_SxCR_DIR_0| DMA_SxCR_MINC |
	                    DMA_SxCR_PSIZE_1 | DMA_SxCR_MSIZE_1 | DMA_SxCR_CIRC;
	DMA2_Stream1->CR |= DMA_SxCR_EN;

	//printf("Tim1: %u Tim8: %u, DMA2-5 %u, DMA2-1: %u\r\n", TIM1->CNT, TIM8->CNT, DMA2_Stream5->NDTR, DMA2_Stream1->NDTR);
...
static void MatrixDecodeColor(uint8_t bytesPerPixel, const uint8_t * pFrame, uint16_t * pR, uint16_t * pG, uint16_t * pB) {
	if (bytesPerPixel == 1) {
		...
		*pR = value & 0x3;
		...
		uint8_t value = *pFrame; //5 bits blue + 3 bits green
		*pB = value >> 3;
		*pG = ((value & 3) << 2);
		value = *(pFrame + 1); //2 bits green + 5 bits red
		*pG |= value >> 5;
		*pR = value & 0x1F;
...
void MatrixBrightness(uint8_t value) {
	g_dimFactor = (255 - value) / MATRIX_DIM_FACTOR_MAX;
