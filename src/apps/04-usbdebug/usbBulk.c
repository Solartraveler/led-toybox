...
Because libusb is licensed under Apache-2.0 too.

Waits for incoming bulk packets and mirrors them back to the outgoing endpoint.
The first byte of each packet is printed on the serial console.
...
#define USB_BULK_QUEUE_TODEVICE_LEN 50
...
bool UsbBulkDequeueToDevice(bulk_t * packet) {
	...
	if (g_bulkState.toDeviceW != thisIndex) {
		...
		uint32_t nextIndex = (thisIndex + 1) % USB_BULK_QUEUE_TODEVICE_LEN;
...
void EndpointBulkToDevice(usbd_device *dev, uint8_t event, uint8_t ep) {
	//printfDirect("Bulk out event %u ep %u\r\n", event, ep);
	if ((ep != USB_ENDPOINT_TODEVICE) || (event != usbd_evt_eprx)) {
		return;
	}
	bulk_t packet;
		...
		case 1:
			...
			*/
			...
			if (!usbd_ep_config(dev, USB_ENDPOINT_TOHOST, epTypeToHost, USB_BULK_BLOCKSIZE)) {
				...
				g_bulkState.toHostFree = UsbTxBytesFree(USB_ENDPOINT_TOHOST) / USB_BULK_BLOCKSIZE;
			}
			if (!usbd_ep_config(dev, USB_ENDPOINT_TODEVICE, epTypeToDevice, USB_BULK_BLOCKSIZE)) {
				result = usbd_fail;
				printfNowait("Error, configure ep to device\r\n");
			} else {
				g_bulkState.epBulkToDeviceEnabled = true;
			}
			usbd_reg_endpoint(dev, USB_ENDPOINT_TODEVICE, &EndpointBulkToDevice);
			usbd_reg_event(dev, usbd_evt_eptx, EndpointEventToHost);
			break;
	}
	return result;
}
...
static usbd_respond usbControl(usbd_device *dev, usbd_ctlreq *req, usbd_rqc_callback *callback) {
	...
	//bulk only reset (specific to mass storage class)
...
bool UsbBulkProcessLoop(void) {
	bulk_t packet;
	uint32_t packets = 0;
	static uint32_t packetsAllTime = 0;
	size_t bytes = 0;
	while (UsbBulkDequeueToDevice(&packet)) {
		bool wasDisabled = g_bulkState.usbIsrDisabled;
		UsbLock();
