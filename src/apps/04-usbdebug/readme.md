# The original USB implementation has troubles with bulk transfers. This program is here to help fixing the lib

It opens two bulk endpoints. Each incoming packet is "processed" by the main loop and then send back
over the outgoing endpoint. The first byte of each packet is printed on the serial port.
This is much slower than USB and therefore makes at some point NAK to the incoming endpoint necessary.
...
And every opening by a user application on the host does a set config.
...
Without the issues above fixed, sending more than USB_BULK_QUEUE_FROMHOST_LEN = 50
...
The debug message "Re-enabled RX ISR" should be seen.

./testbulkreset.py resets the current FIFO index back to 0.
